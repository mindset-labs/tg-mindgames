...
__export(context_exports, {
  HashSetContext: () => HashSetContext,
  createHashSetContext: () => createHashSetContext
});
...
var HashSetNonEmptyBase = class extends import_set_custom.NonEmptyBase {
  ...
  intersect(other) {
    ...
    if ((0, import_custom.isEmptyStreamSourceInstance)(other))
      return this.context.empty();
    const builder = this.context.builder();
    const it = import_stream2.Stream.from(other)[Symbol.iterator]();
    const done = Symbol("Done");
    let value;
  ...
  symDifference(other) {
    ...
    if ((0, import_custom.isEmptyStreamSourceInstance)(other))
      return this;
    const builder = this.toBuilder();
    import_stream2.Stream.from(other).filterNot(builder.remove).forEach(builder.add);
    return builder.build();
  }
  toBuilder() {
...
var HashSetBlockBuilder = class _HashSetBlockBuilder extends BlockBuilderBase {
  constructor(context, source, _entries, _entrySets, size = source?.size ?? 0, level = source?.level ?? 0) {
    ...
    this.addAll = (source) => {
      this.checkLock();
      return import_stream3.Stream.from(source).filterPure(this.add).count() > 0;
    };
    // prettier-ignore
    this.remove = (value) => {
      this.checkLock();
    ...
    this.removeAll = (values) => {
      this.checkLock();
      return import_stream3.Stream.from(values).filterPure(this.remove).count() > 0;
    };
  ...
  prepareMutate() {
    ...
    if (void 0 === this._entrySets) {
      if (void 0 !== this.source && null !== this.source.entrySets) {
        this._entrySets = import_base2.Arr.mapSparse(
