...
var nonleaf_block_exports = {};
__export(nonleaf_block_exports, {
  NonLeafBlock: () => NonLeafBlock
});
module.exports = __toCommonJS(nonleaf_block_exports);
...
var NonLeafBlock = class {
  ...
  stream(reversed = false) {
    return import_stream.Stream.fromArray(this.children, void 0, reversed).flatMap(
      (child) => child.stream(reversed)
    );
  }
  streamRange(range, reversed = false) {
    const indexRange = import_common.IndexRange.getIndicesFor(range, this.length);
    if (indexRange === "all") {
      return import_stream.Stream.fromArray(this.children, void 0, reversed).flatMap(
        (child) => child.stream(reversed)
      );
    ...
    const childStream = import_stream.Stream.fromArray(
      this.children,
