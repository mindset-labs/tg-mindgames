...
var emptyAsyncFastIterator = Object.freeze({
  fastNext(otherwise) {
    return import_common4.AsyncOptLazy.toMaybePromise(otherwise);
  },
  next() {
...
var AsyncOfIterator = class extends AsyncFastIteratorBase {
  ...
  fastNext(otherwise) {
    ...
    if (index >= values.length)
      return import_common4.AsyncOptLazy.toMaybePromise(otherwise);
    return import_common4.AsyncOptLazy.toMaybePromise(values[this.index++]);
  }
};
      ...
      sources.map((source) => {
        if (this.sourcesToClose.has(source)) {
          return source.fastNext(() => {
            this.sourcesToClose.delete(source);
            return import_common4.AsyncOptLazy.toMaybePromise(fillValue);
          });
        }
        return import_common4.AsyncOptLazy.toMaybePromise(fillValue);
      })
...
var AsyncPrependIterator = class extends AsyncFastIteratorBase {
  ...
  fastNext(otherwise) {
    ...
    this.prependDone = true;
    return import_common4.AsyncOptLazy.toMaybePromise(this.item);
  }
};
...
var AsyncMapIterator = class extends AsyncFastIteratorBase {
  constructor(source, mapFun) {
    ...
    this.source = source;
    this.mapFun = mapFun;
    this.state = (0, import_common4.TraverseState)();
    this.return = () => closeIters(source);
  }
  async fastNext(otherwise) {
    const state = this.state;
    ...
    try {
      while (!state.halted && done !== (value = await source.fastNext(done))) {
        const result = await collectFun(
          value,
          state.nextIndex(),
          import_common4.CollectFun.Skip,
          halt
        );
        if (import_common4.CollectFun.Skip === result)
          continue;
        return result;
      }
      return import_common4.AsyncOptLazy.toMaybePromise(otherwise);
...
var AsyncFoldIterator = class extends AsyncFastIteratorBase {
  ...
  async fastNext(otherwise) {
    if (!this.isInitialized) {
      this.current = await import_common4.AsyncOptLazy.toMaybePromise(this.init);
      this.isInitialized = true;
    }
    const state = this.state;
    if (state.halted) {
...
var AsyncReduceIterator = class extends AsyncFastIteratorBase {
  constructor(source, reducer) {
    ...
    this.source = source;
    this.reducer = reducer;
    this.traverseState = (0, import_common4.TraverseState)();
    this.isInitialized = false;
    this.isDone = false;
    this.return = () => closeIters(source);
  }
  async fastNext(otherwise) {
    ...
    const reducer = this.reducer;
    if (!this.isInitialized) {
      this.state = await import_common4.AsyncOptLazy.toMaybePromise(reducer.init);
      this.isInitialized = true;
    }
    ...
    while (done !== (next = await source.fastNext(done))) {
      ...
      if (this.index % skipAmount === 0) {
        ...
        newState.result = await collector.next(
          import_common4.AsyncOptLazy.toMaybePromise(collector.init),
          next,
          0,
          newState.halt
        );
