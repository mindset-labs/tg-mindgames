...
var emptyAsyncFastIterator = Object.freeze({
  fastNext(otherwise) {
    return import_common3.AsyncOptLazy.toMaybePromise(otherwise);
  },
  next() {
...
var AsyncOfIterator = class extends AsyncFastIteratorBase {
  ...
  fastNext(otherwise) {
    ...
    if (index >= values.length)
      return import_common3.AsyncOptLazy.toMaybePromise(otherwise);
    return import_common3.AsyncOptLazy.toMaybePromise(values[this.index++]);
  }
};
...
var AsyncUnfoldIterator = class extends AsyncFastIteratorBase {
  ...
  async fastNext(otherwise) {
    const current = this.current;
    if (import_base3.Token === current)
      return import_common3.AsyncOptLazy.toMaybePromise(otherwise);
    if (this.index === 0) {
      this.index++;
      return current;
    }
    const next = await this.getNext(current, this.index++, import_base3.Token);
    this.current = next;
    if (import_base3.Token === next)
      return import_common3.AsyncOptLazy.toMaybePromise(otherwise);
    return next;
  }
};
      ...
      sources.map((source) => {
        if (this.sourcesToClose.has(source)) {
          return source.fastNext(() => {
            this.sourcesToClose.delete(source);
            return import_common3.AsyncOptLazy.toMaybePromise(fillValue);
          });
        }
        return import_common3.AsyncOptLazy.toMaybePromise(fillValue);
      })
...
var AsyncPrependIterator = class extends AsyncFastIteratorBase {
  ...
  fastNext(otherwise) {
    ...
    this.prependDone = true;
    return import_common3.AsyncOptLazy.toMaybePromise(this.item);
  }
};
...
var AsyncMapIterator = class extends AsyncFastIteratorBase {
  constructor(source, mapFun) {
    ...
    this.source = source;
    this.mapFun = mapFun;
    this.state = (0, import_common3.TraverseState)();
    this.return = () => closeIters(source);
  }
  async fastNext(otherwise) {
    const state = this.state;
    ...
    try {
      while (!state.halted && done !== (value = await source.fastNext(done))) {
        const result = await collectFun(
          value,
          state.nextIndex(),
          import_common3.CollectFun.Skip,
          halt
        );
        if (import_common3.CollectFun.Skip === result)
          continue;
        return result;
      }
      return import_common3.AsyncOptLazy.toMaybePromise(otherwise);
...
var AsyncFoldIterator = class extends AsyncFastIteratorBase {
  ...
  async fastNext(otherwise) {
    if (!this.isInitialized) {
      this.current = await import_common3.AsyncOptLazy.toMaybePromise(this.init);
      this.isInitialized = true;
    }
    const state = this.state;
    if (state.halted) {
...
var AsyncReduceIterator = class extends AsyncFastIteratorBase {
  constructor(source, reducer) {
    ...
    this.source = source;
    this.reducer = reducer;
    this.traverseState = (0, import_common3.TraverseState)();
    this.isInitialized = false;
    this.isDone = false;
    this.return = () => closeIters(source);
  }
  async fastNext(otherwise) {
    ...
    const reducer = this.reducer;
    if (!this.isInitialized) {
      this.state = await import_common3.AsyncOptLazy.toMaybePromise(reducer.init);
      this.isInitialized = true;
    }
  ...
  async fastNext(otherwise) {
    ...
    if (void 0 === this.state) {
      this.state = await Promise.all(
        reducers.map((d) => import_common3.AsyncOptLazy.toMaybePromise(d.init))
      );
    }
    if (this.isDone) {
      await closeIters(this);
...
var AsyncWindowIterator = class extends AsyncFastIteratorBase {
  ...
  async fastNext(otherwise) {
    ...
    while (done !== (next = await source.fastNext(done))) {
      ...
      if (this.index % skipAmount === 0) {
        const newState = {
          result: await import_common3.AsyncOptLazy.toMaybePromise(collector.init),
          size: 1,
          halted: false,
          halt() {
            this.halted = true;
        ...
        newState.result = await collector.next(
          import_common3.AsyncOptLazy.toMaybePromise(collector.init),
          next,
          0,
          newState.halt
        );
...
var AsyncStreamBase = class {
  ...
  async equals(other, eq = import_common4.Eq.objectIs) {
  ...
  async forEach(f, state = (0, import_common4.TraverseState)()) {
    if (state.halted)
      return;
    const done = Symbol("Done");
    let value;
  ...
  async first(otherwise) {
    ...
    if (done === value) {
      return import_common4.AsyncOptLazy.toPromise(otherwise);
    }
  ...
  async countElement(value, eq = import_common4.Eq.objectIs) {
    let result = 0;
    const done = Symbol("Done");
    const iterator = this[Symbol.asyncIterator]();
    let current;
  ...
  async countNotElement(value, eq = import_common4.Eq.objectIs) {
    let result = 0;
    const done = Symbol("Done");
    const iterator = this[Symbol.asyncIterator]();
    let current;
  ...
  indicesOf(searchValue, eq = import_common4.Eq.objectIs) {
  ...
  async indexOf(searchValue, occurrance = 1, eq = import_common4.Eq.objectIs) {
    if (occurrance <= 0)
      return void 0;
    const done = Symbol("Done");
    let value;
  ...
  async containsSlice(source, eq = import_common4.Eq.objectIs) {
    const iterator = this[Symbol.asyncIterator]();
  ...
  min(otherwise) {
    return this.minBy(import_common4.Comp.defaultComp().compare, otherwise);
  }
  ...
  max(otherwise) {
    return this.maxBy(import_common4.Comp.defaultComp().compare, otherwise);
  }
  ...
  splitOn(sepElem, eq = import_common4.Eq.objectIs) {
    return new AsyncSplitOnStream(this, sepElem, eq);
  }
  distinctPrevious(eq = import_common4.Eq.objectIs) {
    return new AsyncDistinctPreviousStream(this, eq);
  }
  window(windowSize, skipAmount = windowSize, collector = import_common4.AsyncReducer.toArray()) {
    return new AsyncWindowStream(this, windowSize, skipAmount, collector);
  }
  async fold(init, next) {
    let current = await import_common4.AsyncOptLazy.toMaybePromise(init);
    const state = (0, import_common4.TraverseState)();
    const done = Symbol("done");
    const iterator = this[Symbol.asyncIterator]();
    let value;
    try {
  ...
  async reduce(reducer) {
    const traverseState = (0, import_common4.TraverseState)();
    const next = reducer.next;
    let state = await import_common4.AsyncOptLazy.toMaybePromise(reducer.init);
    const done = Symbol("Done");
    let value;
    const iter = this[Symbol.asyncIterator]();
    try {
  ...
  async reduceAll(...reducers) {
    let state = await Promise.all(
      reducers.map((d) => import_common4.AsyncOptLazy.toMaybePromise(d.init))
    );
...
var AsyncPrependStream = class extends AsyncStreamBase {
  ...
  async first() {
    return import_common4.AsyncOptLazy.toPromise(this.item);
  }
  async last() {
    return this.source.last(this.item);
  }
...
var AsyncEmptyStream = class extends AsyncStreamBase {
  ...
  transform(transformer) {
    return AsyncStreamConstructorsImpl.from(
      async () => await transformer.stateToResult(
        await import_common4.AsyncOptLazy.toMaybePromise(transformer.init)
      )
    );
  }
  filter() {
  ...
  fold(init) {
    return import_common4.AsyncOptLazy.toPromise(init);
  }
  foldStream() {
    return this;
  }
  async reduce(reducer) {
    return reducer.stateToResult(await import_common4.AsyncOptLazy.toPromise(reducer.init));
  }
  reduceStream() {
    return this;
  }
  reduceAll(...reducers) {
    return Promise.all(
      reducers.map(
        async (p) => p.stateToResult(await import_common4.AsyncOptLazy.toMaybePromise(p.init))
      )
    );
  }
  reduceAllStream() {
...
var Transformer;
((Transformer2) => {
  Transformer2.window = (windowSize, skipAmount = windowSize, collector = import_common6.Reducer.toArray()) => {
    return import_common6.Reducer.create(
      () => /* @__PURE__ */ new Set(),
      (state, elem, index) => {
        for (const current of state) {
          if (current.size >= windowSize || current.halted) {
        ...
        if (index % skipAmount === 0) {
          const newState = {
            result: (0, import_common6.OptLazy)(collector.init),
            size: 1,
            halted: false,
            halt() {
              this.halted = true;
          ...
          newState.result = collector.next(
            (0, import_common6.OptLazy)(collector.init),
            elem,
            0,
            newState.halt
          );
