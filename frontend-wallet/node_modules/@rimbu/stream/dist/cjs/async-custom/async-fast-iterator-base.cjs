...
var async_fast_iterator_base_exports = {};
__export(async_fast_iterator_base_exports, {
...
module.exports = __toCommonJS(async_fast_iterator_base_exports);
...
var Transformer;
((Transformer2) => {
  Transformer2.window = (windowSize, skipAmount = windowSize, collector = import_common3.Reducer.toArray()) => {
    return import_common3.Reducer.create(
      ...
      (state, elem, index) => {
        ...
        if (index % skipAmount === 0) {
          const newState = {
            result: (0, import_common3.OptLazy)(collector.init),
            size: 1,
            halted: false,
            halt() {
              this.halted = true;
          ...
          newState.result = collector.next(
            (0, import_common3.OptLazy)(collector.init),
            elem,
            0,
            newState.halt
          );
  ...
  function distinctPrevious(eq = import_common3.Eq.objectIs) {
    return import_common3.Reducer.create(
      () => [],
      (current, elem) => {
        current.push(elem);
...
((AsyncTransformer2) => {
  AsyncTransformer2.window = (windowSize, skipAmount = windowSize, collector = import_common4.AsyncReducer.toArray()) => {
    return import_common4.AsyncReducer.create(
      () => /* @__PURE__ */ new Set(),
      async (state, elem, index) => {
        ...
        if (index % skipAmount === 0) {
          const newState = {
            result: await import_common4.AsyncOptLazy.toMaybePromise(collector.init),
            size: 1,
            halted: false,
            halt() {
              this.halted = true;
          ...
          newState.result = collector.next(
            await import_common4.AsyncOptLazy.toMaybePromise(collector.init),
            elem,
            0,
            newState.halt
          );
...
var emptyAsyncFastIterator = Object.freeze({
  fastNext(otherwise) {
    return import_common6.AsyncOptLazy.toMaybePromise(otherwise);
  },
  next() {
...
var AsyncOfIterator = class extends AsyncFastIteratorBase {
  ...
  fastNext(otherwise) {
    ...
    if (index >= values.length)
      return import_common6.AsyncOptLazy.toMaybePromise(otherwise);
    return import_common6.AsyncOptLazy.toMaybePromise(values[this.index++]);
  }
};
...
var AsyncUnfoldIterator = class extends AsyncFastIteratorBase {
  ...
  async fastNext(otherwise) {
    const current = this.current;
    if (import_base4.Token === current)
      return import_common6.AsyncOptLazy.toMaybePromise(otherwise);
    if (this.index === 0) {
      this.index++;
      return current;
    }
    const next = await this.getNext(current, this.index++, import_base4.Token);
    this.current = next;
    if (import_base4.Token === next)
      return import_common6.AsyncOptLazy.toMaybePromise(otherwise);
    return next;
  }
};
      ...
      sources.map((source) => {
        if (this.sourcesToClose.has(source)) {
          return source.fastNext(() => {
            this.sourcesToClose.delete(source);
            return import_common6.AsyncOptLazy.toMaybePromise(fillValue);
          });
        }
        return import_common6.AsyncOptLazy.toMaybePromise(fillValue);
      })
...
var AsyncPrependIterator = class extends AsyncFastIteratorBase {
  ...
  fastNext(otherwise) {
    ...
    this.prependDone = true;
    return import_common6.AsyncOptLazy.toMaybePromise(this.item);
  }
};
...
var AsyncMapIterator = class extends AsyncFastIteratorBase {
  constructor(source, mapFun) {
    ...
    this.source = source;
    this.mapFun = mapFun;
    this.state = (0, import_common6.TraverseState)();
    this.return = () => closeIters(source);
  }
  async fastNext(otherwise) {
    const state = this.state;
    ...
    try {
      while (!state.halted && done !== (value = await source.fastNext(done))) {
        const result = await collectFun(
          value,
          state.nextIndex(),
          import_common6.CollectFun.Skip,
          halt
        );
        if (import_common6.CollectFun.Skip === result)
          continue;
        return result;
      }
      return import_common6.AsyncOptLazy.toMaybePromise(otherwise);
...
var AsyncFoldIterator = class extends AsyncFastIteratorBase {
  ...
  async fastNext(otherwise) {
    if (!this.isInitialized) {
      this.current = await import_common6.AsyncOptLazy.toMaybePromise(this.init);
      this.isInitialized = true;
    }
    const state = this.state;
    if (state.halted) {
...
var AsyncReduceIterator = class extends AsyncFastIteratorBase {
  constructor(source, reducer) {
    ...
    this.source = source;
    this.reducer = reducer;
    this.traverseState = (0, import_common6.TraverseState)();
    this.isInitialized = false;
    this.isDone = false;
    this.return = () => closeIters(source);
  }
  async fastNext(otherwise) {
    ...
    const reducer = this.reducer;
    if (!this.isInitialized) {
      this.state = await import_common6.AsyncOptLazy.toMaybePromise(reducer.init);
      this.isInitialized = true;
    }
    ...
    while (done !== (next = await source.fastNext(done))) {
      ...
      if (this.index % skipAmount === 0) {
        ...
        newState.result = await collector.next(
          import_common6.AsyncOptLazy.toMaybePromise(collector.init),
          next,
          0,
          newState.halt
        );
