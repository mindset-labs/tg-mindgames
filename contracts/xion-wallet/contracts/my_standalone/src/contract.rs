...
pub fn instantiate(
    ...
    MY_STANDALONE.instantiate(deps.branch(), info, StandaloneInstantiateMsg {}, is_migratable)?;
...
pub fn execute(
    ...
    match msg {
        MyStandaloneExecuteMsg::UpdateConfig {} => update_config(deps, env, info, standalone),
        MyStandaloneExecuteMsg::Increment {} => increment(deps, standalone),
        MyStandaloneExecuteMsg::Reset { count } => reset(deps, env, info, count, standalone),
        MyStandaloneExecuteMsg::IbcCallback(msg) => {
            let binding = MY_STANDALONE;
...
fn update_config(deps: DepsMut, env: Env, info: MessageInfo, module: MyStandalone) -> MyStandaloneResult {
    MY_STANDALONE
        .admin
...
fn increment(deps: DepsMut, module: MyStandalone) -> MyStandaloneResult {
