...
//!
//! ```bash
//! $ just publish my-standalone uni-6 osmo-test-5
//! ```
...
fn publish(networks: Vec<ChainInfo>) -> anyhow::Result<()> {
    ...
    for network in networks {
        ...
        let publisher_acc = abstract_client.fetch_or_build_account(standalone_namespace, |builder| builder.namespace(Namespace::from_id(MY_STANDALONE_ID).unwrap()))?;
