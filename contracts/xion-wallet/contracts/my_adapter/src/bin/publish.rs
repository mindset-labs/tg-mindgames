...
//!
//! ```bash
//! $ just publish my-adapter uni-6 osmo-test-5
//! ```
use my_adapter::{
...
fn publish(networks: Vec<ChainInfo>) -> anyhow::Result<()> {
    ...
    for network in networks {
        ...
        let publisher_acc = abstract_client.fetch_or_build_account(adapter_namespace, |builder| builder.namespace(Namespace::from_id(MY_ADAPTER_ID).unwrap()))?;
